...
if [ "$TRAIN_EVAL_SPLIT" = "test" ]; then
  TRAIN_EVAL_JSON="trm_agent/processed/${DATASET}/test.jsonl"
  TRAIN_QUERY_EVAL_NPY="${EMB_DIR}/query_test.npy"
else
  TRAIN_EVAL_JSON="trm_agent/processed/${DATASET}/dev.jsonl"
  TRAIN_QUERY_EVAL_NPY="${EMB_DIR}/query_dev.npy"
fi
...
CMD+=(
  ...
  dev_json="$TRAIN_EVAL_JSON"
  query_emb_dev_npy="$TRAIN_QUERY_EVAL_NPY"
  eval_json="$TRAIN_EVAL_JSON"
  query_emb_eval_npy="$TRAIN_QUERY_EVAL_NPY"
