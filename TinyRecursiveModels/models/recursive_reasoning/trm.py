...
class TinyRecursiveReasoningModel_ACTV1_Inner(nn.Module):
    def __init__(self, config: TinyRecursiveReasoningModel_ACTV1Config) -> None:
        ...
        # External embedding holders (injected by train script)
        self.puzzle_emb_data = None
        self.relation_emb_data = None
        if self.config.puzzle_emb_ndim > 0:
            # Fallback trainable sparse embedding (used when external npy is not injected)
            ...
            if self.config.puzzle_emb_ndim != self.config.hidden_size:
        ...
        if self.config.relation_emb_ndim > 0 and self.config.relation_emb_ndim != self.config.hidden_size:
    ...
    def puzzle_emb_lookup(self, puzzle_identifiers: torch.Tensor) -> torch.Tensor:
        if self.puzzle_emb_data is not None:
            ids_cpu = puzzle_identifiers.cpu()
            if isinstance(self.puzzle_emb_data, torch.Tensor):
                emb = F.embedding(puzzle_identifiers.to(self.puzzle_emb_data.device), self.puzzle_emb_data)
            else:
        ...
        elif hasattr(self, "puzzle_emb"):
            emb = self.puzzle_emb(puzzle_identifiers).to(self.forward_dtype)
        else:
            emb = torch.zeros((*puzzle_identifiers.shape, self.config.hidden_size), device=puzzle_identifiers.device, dtype=self.forward_dtype)

        if self.puzzle_projector is not None:
    ...
    def relation_emb_lookup(self, relation_identifiers: torch.Tensor) -> torch.Tensor:
        if self.relation_emb_data is None:
            return torch.zeros((*relation_identifiers.shape, self.config.hidden_size), device=relation_identifiers.device, dtype=self.forward_dtype)
        ids_cpu = relation_identifiers.cpu()
        if isinstance(self.relation_emb_data, torch.Tensor):
            emb = F.embedding(relation_identifiers.to(self.relation_emb_data.device), self.relation_emb_data)
        else:
        ...
        if self.relation_projector is not None:
    ...
    def _input_embeddings(self, input: torch.Tensor, puzzle_identifiers: torch.Tensor, relation_identifiers: torch.Tensor = None):
        ...
        if self.config.puzzle_emb_ndim > 0:
            ...
            if relation_identifiers is not None:
                puzzle_embedding = puzzle_embedding + self.relation_emb_lookup(relation_identifiers)
            puzzle_embedding = puzzle_embedding.view(puzzle_embedding.shape[0], -1, self.config.hidden_size)
            embedding = torch.cat((puzzle_embedding, embedding), dim=-2)

        # Position embeddings
        if self.config.pos_encodings == "learned":
            # scale by 1/sqrt(2) to maintain forward variance
            cur_len = embedding.size(1)
            embedding = 0.707106781 * (embedding + self.embed_pos.embedding_weight[:cur_len].to(self.forward_dtype))
    ...
    def reset_carry(self, reset_flag: torch.Tensor, carry: TinyRecursiveReasoningModel_ACTV1InnerCarry, target_seq_len: int = None):
        ...
        if target_seq_len is not None and carry.z_H.shape[1] != target_seq_len:
            def _resize(t):
                ...
                return new_t
            carry = TinyRecursiveReasoningModel_ACTV1InnerCarry(z_H=_resize(carry.z_H), z_L=_resize(carry.z_L))

        return TinyRecursiveReasoningModel_ACTV1InnerCarry(
            z_H=torch.where(reset_flag.view(-1, 1, 1), self.H_init, carry.z_H.to(device).clone()),
            z_L=torch.where(reset_flag.view(-1, 1, 1), self.L_init, carry.z_L.to(device).clone()),
        )
    ...
    def forward(self, carry: TinyRecursiveReasoningModel_ACTV1InnerCarry, batch: Dict[str, torch.Tensor]) -> Tuple[TinyRecursiveReasoningModel_ACTV1InnerCarry, torch.Tensor, Tuple[torch.Tensor, torch.Tensor], torch.Tensor]:
        ...
        if current_len != target_len:
            def _force_resize(t):
                ...
                new_t[:, :copy_len, :] = t[:, :copy_len, :]
                return new_t
            carry = TinyRecursiveReasoningModel_ACTV1InnerCarry(z_H=_force_resize(carry.z_H), z_L=_force_resize(carry.z_L))
...
class TinyRecursiveReasoningModel_ACTV1(nn.Module):
    ...
    def forward(self, carry: TinyRecursiveReasoningModel_ACTV1Carry, batch: Dict[str, torch.Tensor]) -> Tuple[TinyRecursiveReasoningModel_ACTV1Carry, Dict[str, torch.Tensor]]:
        ...
        current_seq_len = (
            batch["input_ids"].shape[1] + self.inner.puzzle_emb_len
            if "input_ids" in batch else
            batch["inputs"].shape[1] + self.inner.puzzle_emb_len
        )
